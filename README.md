...
可為各 CSV 標記實驗條件與任務、匿名化參與者，並選擇組間或組內設計。除行為比例、修改率、決策時間、建議規模、參與者比較與時間趨勢外，工具也提供 Wilson 95% 信賴區間、非參數條件檢定、效果量與決策序列。

「下載完整 ZIP 報表」包含清理後事件、參與者／條件／時間／序列摘要、資料品質報告、分析設定、易讀 HTML 報告、欄位字典及 Python／R 範例。圖表直接嵌在 HTML 報告內，可查看或自行截圖。若設定組間或組內研究設計，ZIP 會再加入對應的統計檢定與效果量。這些結果仍須搭配任務表現、問卷或訪談解讀，正式推論建議用專業統計軟體複核。
