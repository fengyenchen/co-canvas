...
- 多影片與音訊節點、片段時間區間及 Gemini 分析
- 文件／資料與圖片節點、公開直連、本機預覽與 Gemini 附件分析
...
- 專案列表：擁有者的專案、以 Email 受邀的私人專案，以及登入後曾開啟的公開專案會出現在列表；非擁有者可只從自己的列表移除，再次開啟連結時會重新出現。
...
- 音訊節點支援本機或公開直連的 MP3、WAV、M4A、AAC、OGG、FLAC，可播放、設定時間區間並交給 Gemini 分析；上限為 100 MB。
...
- 文件／資料節點支援 PDF、TXT／Markdown／CSV／JSON、DOCX、XLSX、PPTX。
