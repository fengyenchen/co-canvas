...
import { formatMediaDuration } from '../../utils/mediaTime'
...
export function AudioNode({ data, selected }: NodeProps<AudioCanvasNode>) {
    const subtitle = data.content || data.fileName || data.source
    ...
    return (
        <div className={`w-64 rounded-xl border bg-background px-4 py-3 shadow-sm ${selected ? 'border-primary ring-2 ring-primary/15' : 'border-border'}`}>
            ...
            <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                    <div className="wrap-break-word font-semibold text-foreground">{data.title}</div>
                    {subtitle && <div className="mt-1 line-clamp-2 wrap-break-word text-sm text-foreground/65">{subtitle}</div>}
                    {data.durationMs && <div className="mt-1 text-xs tabular-nums text-foreground/50">{formatMediaDuration(data.durationMs)}</div>}
                </div>
            </div>
            <Handle id="bottom" type="source" position={Position.Bottom} className={handleClassName} />
        </div>
